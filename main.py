...
class InstrScr(Screen):
    def __init__(self, **kwargs):
        ...
        lbl_name = Label(text="Введіть імʼя:", halign='right', color=lbl_color, bold=True, font_size=40)
...
class PulseScr(Screen):
    def __init__(self, **kwargs):
        ...
        lbl_result = Label(text="Введіть результат:", halign='right', color=lbl_color, bold=True, font_size=40)
        self.input_result = TextInput(text="1", multiline=False)
        self.input_result.set_disabled(True)
        self.btn = Button(text='Почати', size_hint=(.3, .2), pos_hint={'center_x': .5}, bold=True,
